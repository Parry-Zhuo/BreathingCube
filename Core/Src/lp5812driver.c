...
int LP5812_DetectFault(void){
    uint16_t fault_status;

    // Step 1: Read the TSD_CONFIG_STATUS register (0x1A)
    fault_status = LP5812_ReadRegister(0x1A);

    // Step 2: Check for thermal shutdown fault (Bit 7)
    if (fault_status & 0x80) {
        // printf("Thermal shutdown detected!\n");
        // Handle thermal shutdown fault (e.g., reduce load, disable LEDs)
    }

    // Step 3: Check for open circuit faults (Bits [6:4])
    if (fault_status & 0x70) {
        // printf("Open circuit fault detected on LEDs:\n");
        if (fault_status & 0x10) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
        if (fault_status & 0x20) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
        if (fault_status & 0x40) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
    }

    // Step 4: Check for short circuit faults (Bits [3:0])
    if (fault_status & 0x0F) {
        // printf("Short circuit fault detected on LEDs:\n");
        if (fault_status & 0x01) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
        if (fault_status & 0x02) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
        if (fault_status & 0x04) HAL_GPIO_WritePin(GPIOA, Indication_light_Pin, GPIO_PIN_SET);
    }
}
void ClearAllFaults() {
    HAL_StatusTypeDef status;

    // Clear all faults (LOD, LSD, TSD)
    status = LP5812_WriteRegister(0x22, 0x07);
    if (status == HAL_OK) {
